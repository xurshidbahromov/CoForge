...
async def list_tasks(project_id: int, session=Depends(get_async_session)):
...
async def update_task(task_id: int, task_update: dict, session=Depends(get_async_session)):
